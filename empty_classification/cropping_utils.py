...
export_file_path = '/Users/deregowskis/Desktop/work/jsw/empty_full/export_new_images_to_crop.jsonl'
image_input_folder_path = '/Users/deregowskis/Desktop/work/jsw/empty_full/original_images/'
image_cropped_output_folder_path = '/Users/deregowskis/Desktop/work/jsw/empty_full/new_cropped/'
image_merged_output_folder_path = '/Users/deregowskis/Desktop/work/jsw/empty_full/merged/'
...
def prepareCropList(line_number, export_path=export_file_path):
    ...
    with open(export_path) as f:
        ...
        truck_json = \
            line.strip('{"imageGcsUri":"gs://wozy_jsw/' + image_name + '","boundingBoxAnnotations":').split("]", 1)[
                0] + ']'
        ...
        for i in range(len(truck_json)):
            ...
            # Fixed bug
            if truck_json[i] != [']']:
                if displayName != -1 and xMin != -1 and xMax != -1 and yMin != -1 and yMax != -1:
...
def cropBiggestImage(biggestarea, image_input_folder_path=image_input_folder_path,
    ...
    if kOptions == 0:
        newName = imageOrgName.strip('.png') + '_' + displayName + '_' + '.png'
        saveName = cropped_output_folder_path + newName
        cropped_im.save(saveName)
        # write records to jsonl
        with open(cropped_output_folder_path+'merged.jsonl', 'a+') as f:
            # string1 + newName
            string1 = '{"imageGcsUri":"gs://wozy_jsw/'
